...
//function prototype
double **perceptron(double *inputArrPT, double *weightPT, double bias, int itr, FILE* outputFile, int row);
void unpackage(double **inputArrPT);
double *weightGenerate();
double randFloat(double min, double max);
void training(double mae, double *sigPT, double *outputTrainPT, double *sumPT, double *inputTrainPT, int itr, double *weightPT, double *bias, FILE* outputFile);

double **perceptron(double *inputTrainPT, double *weightPT, double bias, int itr, FILE* outputFile, int row)
{
    double *sumPT;
    double *sigPT;
    double mae;
    ...
    sumPT = lineReg(inputTrainPT,weightPT,bias,row);
    returnArr[0] = sumPT;
